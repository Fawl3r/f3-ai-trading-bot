#!/usr/bin/env python3
"""
PERFECT 75% BALANCE AI TRADER
Finding the exact sweet spot for 75%+ win rate
"""
...
class Perfect75Balance:
    def __init__(self, initial_balance: float = 200.0):
        self.initial_balance = initial_balance
        
        # PERFECT 75% BALANCE CONFIGURATION
        self.config = {
            "leverage_min": 4, "leverage_max": 6,
            "position_min": 0.09, "position_max": 0.16,
            
            # PERFECT RISK/REWARD BALANCE
            "profit_target_min": 0.020,  # 2.0% minimum
            "profit_target_max": 0.038,  # 3.8% maximum
            "stop_loss": 0.022,          # 2.2% stop (balanced)
            "trail_distance": 0.006,     # 0.6% trail
            "trail_start": 0.012,        # Start at 1.2%
            
            # PERFECT STRICTNESS BALANCE
            "min_conviction": 87,        # 87% conviction (sweet spot)
            "max_daily_trades": 4,       # Balanced frequency
            "max_hold_hours": 12,        # Balanced holds
            "confluence_required": 5,    # 5+ confluences (achievable)
            "volume_threshold": 1.45,    # Achievable volume
        }
        
        print("🎯 PERFECT 75% BALANCE AI TRADER")
        print("⚡ EXACT SWEET SPOT FOR 75%+ WIN RATE")
        print("=" * 50)
    ...
    def perfect_75_analysis(self, indicators: dict) -> dict:
        if not indicators:
        ...
        # PERFECT LONG SETUP (balanced thresholds)
        if (rsi <= 36 and indicators['rsi_momentum'] > 1.2 and indicators['rsi_slope'] > 0.8 and
            indicators['price_position'] <= 0.38 and indicators['support_distance'] <= 0.022 and
            indicators['lower_wick_pct'] >= 0.28):
            ...
            if indicators['ema_bullish'] and indicators['ema9_vs_ema21'] > 0.002:
                confluences.append('strong_ema_bullish')
                conviction += 24
                reasoning.append("Strong EMA bullish alignment")
            elif indicators['price_vs_ema21'] > -0.020:
                confluences.append('ema21_support')
                conviction += 17
                reasoning.append("EMA21 support")
            
            if indicators['macd_bullish'] and indicators['macd_strength'] > 0.010:
            ...
            if indicators['support_distance'] <= 0.012:
        ...
        # PERFECT SHORT SETUP (balanced thresholds)
        elif (rsi >= 64 and indicators['rsi_momentum'] < -1.2 and indicators['rsi_slope'] < -0.8 and
              indicators['price_position'] >= 0.62 and indicators['resistance_distance'] <= 0.022 and
              indicators['upper_wick_pct'] >= 0.28):
            ...
            if indicators['ema_bearish'] and indicators['ema9_vs_ema21'] < -0.002:
            ...
            if not indicators['macd_bullish'] and indicators['macd_strength'] > 0.010:
            ...
            if indicators['resistance_distance'] <= 0.012:
        ...
        # PERFECT BALANCE FILTERS
        if indicators['volume_ratio'] < 1.25:
            return {'trade': False, 'direction': None, 'conviction': 0, 'confluences': []}
        ...
        if indicators['volume_ratio'] >= 2.2:
            conviction += 6
    ...
    def run_perfect_test(self, data: pd.DataFrame) -> dict:
        print("🎯 RUNNING PERFECT 75% BALANCE TEST")
        print("⚡ EXACT SWEET SPOT FOR 75%+ WIN RATE")
        print("=" * 50)
        ...
        for i in range(65, len(data) - 100):
            ...
            analysis = self.perfect_75_analysis(indicators)
            if not analysis['trade']:
                continue
            
            entry_price = data.iloc[i]['close']
            conv_factor = (analysis['conviction'] - 87) / 11
        ...
        print(f"\n{'='*70}")
        print(f"🎯 PERFECT 75% BALANCE RESULTS")
        print(f"{'='*70}")
...
def main():
    print("🎯 PERFECT 75% BALANCE AI VISUAL TRADER")
    print("⚡ FINDING THE EXACT SWEET SPOT")
    print("🔧 BALANCED: Not Too Strict, Not Too Loose")
    print("🏆 TARGET: 75%+ Win Rate with Consistent Trades")
    print("=" * 60)
    
    perfect_trader = Perfect75Balance(200.0)
    data = perfect_trader.generate_market_data(60)
    results = perfect_trader.run_perfect_test(data)
